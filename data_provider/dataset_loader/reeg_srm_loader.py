...
def get_id_list_reeg_srm(args, label_path, a=0.6, b=0.8):
    '''
    Loads subject IDs for all, training, validation, and test sets for REEG-SRM data
    Args:
        args: arguments
...
class REEGSRMLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        a, b = 0.6, 0.8
        self.all_ids, self.train_ids, self.val_ids, self.test_ids = get_id_list_reeg_srm(args, self.label_path, a, b)
        if flag == 'TRAIN':
            ids = self.train_ids
            print('train ids:', ids)
        elif flag == 'VAL':
