...
from utils.losses import ts2vec_loss
...
class Exp_Pretrain_TS2Vec(Exp_Basic):
    ...
    def encode(self, loader):
        ...
        with torch.no_grad():
            for i, (batch_x, label_id, padding_mask) in enumerate(loader):
                ...
                # Use pooling to get denser representation for linear probing
                if self.swa:
                    _, reprs_z = self.swa_model(batch_x, padding_mask, None, None)
                else:
                    _, reprs_z = self.model(batch_x, padding_mask, None, None)
                reprs_z = reprs_z.reshape(reprs_z.shape[0], -1)
                reprs_list.append(reprs_z)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label_id, padding_mask) in enumerate(pretrain_loader):
                ...
                # TS2Vec method does not utilize project head but use representation directly
                reprs_h_1, _ = self.model(batch_x, padding_mask, None, None)
                reprs_h_2, _ = self.model(batch_x, padding_mask, None, None)
                # cross contrast
                loss = criterion(reprs_h_1, reprs_h_2)
