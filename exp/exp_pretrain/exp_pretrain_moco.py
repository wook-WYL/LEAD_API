...
class Exp_Pretrain_MOCO(Exp_Basic):
    def __init__(self, args):
        ...
        # --------------------
        # MoCo related configs
        # --------------------
        self.m_model = copy.deepcopy(self.model).to(self.device)  # same initialization as model
        self.momentum = args.momentum
        self.K = args.K  # queue size
        self.T = args.temperature  # temperature
    ...
    def _dequeue_and_enqueue(self, keys):
        # keys: [N, C]
        queue = self.model.module.queue
        queue_ptr = self.model.module.queue_ptr
        ...
        if end_ptr <= self.K:
            # queue is not full
            queue[:, ptr:end_ptr] = keys.T
        else:
            # split into two parts
            first_part = self.K - ptr
            second_part = end_ptr - self.K
            # queue first part to the end of the queue
            queue[:, ptr:self.K] = keys[:first_part].T
            # queue second part to the beginning of the queue
            queue[:, 0:second_part] = keys[first_part:].T

        # update pointer
        queue_ptr[0] = end_ptr % self.K
        self.model.module.queue = queue
        self.model.module.queue_ptr = queue_ptr

    @torch.no_grad()
    def _update_momentum_encoder(self):
        for param_q, param_k in zip(self.model.parameters(), self.m_model.parameters()):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label_id, padding_mask) in enumerate(pretrain_loader):
                ...
                # q from self.model (query encoder)
                q_repr_h, q_repr_z = self.model(batch_x, padding_mask, None, None)
                q_repr_z = F.normalize(q_repr_z, dim=1)

                # k from self.m_model (key encoder)
                with torch.no_grad():
                    _, k_repr_z = self.m_model(batch_x, padding_mask, None, None)
                    k_repr_z = F.normalize(k_repr_z, dim=1)

                # MoCo Loss
                loss = criterion(q_repr_z, k_repr_z, self.model.module.queue.clone().detach(), self.T)
                ...
                # update the queue
                self._dequeue_and_enqueue(k_repr_z)
