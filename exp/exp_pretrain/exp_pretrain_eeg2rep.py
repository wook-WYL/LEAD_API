...
from utils.tools import off_diagonal
import copy
...
class Exp_Pretrain_EEG2Rep(Exp_Basic):
    ...
    def _update_momentum_encoder(self):
        for param_q, param_k in zip(self.model.module.encoder.parameters(),
                                    self.model.module.m_encoder.parameters()):
    ...
    def encode(self, loader):
        ...
        with torch.no_grad():
            for i, (batch_x, label_id, padding_mask) in enumerate(loader):
                ...
                if self.swa:
                    _, repr_m_p = self.swa_model(batch_x, padding_mask, None, None)
                else:
                    _, repr_m_p = self.model(batch_x, padding_mask, None, None)
                repr_m_p = torch.mean(repr_m_p, dim=1)
                reprs_list.append(repr_m_p)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label_id, padding_mask) in enumerate(pretrain_loader):
                ...
                # encoder
                repr_m, repr_m_p = self.model(batch_x, padding_mask, None, None)

                # EEG2Rep Loss
                # target encoder should use the same mask as context encoder
                align_loss = F.mse_loss(repr_m, repr_m_p)
                y = self.gap(repr_m_p.transpose(2, 1)).squeeze()
                y = y - y.mean(dim=0)

                std_y = torch.sqrt(y.var(dim=0) + 0.0001)
                std_loss = torch.mean(F.relu(1 - std_y))

                cov_y = (y.T @ y) / (len(label_id) - 1)
                cov_loss = off_diagonal(cov_y).pow_(2).sum().div(y.shape[-1])
                loss = align_loss + std_loss + cov_loss
